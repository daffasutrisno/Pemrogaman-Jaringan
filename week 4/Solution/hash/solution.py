...
if __name__ == '__main__':
    if len(sys.argv) == 2 and sys.argv[1] == 'run':
        digest1 = hash_dict(test_data)
        digest2 = hashlib.sha256(str(test_data).encode('utf-8')).hexdigest()
        assert_true_hashes(digest1, digest2)

        modified = test_data.copy()
        modified['age'] = 31
        digest3 = hash_dict(modified)
        assert_true_hashes(digest1, digest3)
    else:
